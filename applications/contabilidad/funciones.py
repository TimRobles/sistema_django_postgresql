...
def calcular_datos_boleta(obj):
    ...
    obj.essalud = ((obj.haber_mensual + obj.compra_vacaciones) * essalud.porcentaje).quantize(Decimal('0.01'))
    if essalud.ley30334 and obj.tipo == 2: #GRATIFICACIÓN
        obj.ley29351 = ((obj.haber_mensual + obj.compra_vacaciones) * essalud.porcentaje).quantize(Decimal('0.01'))
        obj.essalud = Decimal('0.00')

    obj.aporte_obligatorio = ((obj.haber_mensual + obj.compra_vacaciones) * aporte_obligatorio).quantize(Decimal('0.01'))
    if datos_planilla.tipo_comision == 1:
        obj.comision = ((obj.haber_mensual + obj.compra_vacaciones) * comision_flujo_mixta).quantize(Decimal('0.01'))
    else:
        obj.comision = ((obj.haber_mensual + obj.compra_vacaciones) * comision_flujo).quantize(Decimal('0.01'))
    obj.prima_seguro = ((obj.haber_mensual + obj.compra_vacaciones) * prima_seguro).quantize(Decimal('0.01'))
