...
def movimientos_bancarios(cuenta_bancaria, fecha=None, monto=None, numero_operacion=None, comentario=None, es_pago=None, foto=None):
    movimientos = []
    ...
    if es_pago == '2':
        ingresos = ingresos.filter(es_pago=True)
    elif es_pago == '3':
        ingresos = ingresos.filter(es_pago=False)
    if foto == '2':
